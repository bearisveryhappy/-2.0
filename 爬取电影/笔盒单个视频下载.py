...
def sum(file_name):
    with open(f"电影/{file_name}.mp4", "wb") as f:
        with open("第二层m3u8文件.txt", "r") as f1:
            for line in f1:
                ...
                if line.startswith("#") | line.startswith('https'):
                    continue
                try:
                    with open(f"ts_viode_02/temp_{line.split('/')[-1]}", "rb") as f2:
                        f.write(f2.read())
...
def mian(url):
    ...
    # head_second_link = second_urls.split("//20")[0]
    # 下载ts文件
    asyncio.run(aio_file_download(file_links))
    # 获取密码
